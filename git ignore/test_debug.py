...
limits = DesignLimits(
    ...
    min_collective_deg=-10, max_collective_deg=65, reserve_fuel_kg=p.RESERVE_FUEL_KG
)
planner = MissionPlanner(
    rotor=rotor, airfoil_provider=p.AIRFOIL_PROVIDER, num_rotors=p.NUM_ENGINES, 
    empty_mass_kg=p.EMPTY_MASS_KG + p.PAYLOAD_MASS_KG, fuel_mass_kg=p.FUEL_MASS_KG,
    power_model=power_model, fuel_model=fuel_model, limits=limits, flat_plate_area_m2=p.FLAT_PLATE_AREA_M2
)

def step_cb(state):
    log(f"Time: {state.time_s}, Fuel: {state.fuel_mass_kg}")
planner.step_callback = step_cb
...
for seg_data in p.MISSION_PLAN:
    ...
    profile.append(MissionSegment(
        name=name, seg_type=enum_type, duration_s=dur, altitude_m=alt, rpm=rpm, 
        collective_deg=coll, vertical_speed_mps=v_speed, cruise_speed_mps=c_speed, dt_s=dt
    ))

try:
    log("Running mission...")
    planner.run_mission(profile)
    log("Success")
